...
1. 栈（stack）：存储寄存器和局部变量，从用户地址空间高地址向下拓展；
2. 堆（heap）：存储动态数据（数组和链表）；
3. 静态数据段（static data segment）：存储常量和其他静态变量；
4. 代码段（text segment）：存储执行代码；
5. 保留段：保留的存储空间。

![image-20231113162503603](cod2_5.assets/image-20231113162503603.png)

>**内存泄漏：**
>
>C通过显示函数调用来分配和释放堆上的内存，`malloc`分配，`free`释放；忘记释放空间会导致**内存泄露**，最终耗尽大量内存；过早释放空间会导致**悬空指针**，会导致指针乱指。

RISC-V 寄存器约定：

![image-20231113162835472](cod2_5.assets/image-20231113162835472.png)
